...
void ExN02PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
    ...
    if(fGeneratorType=="Generator"){ // Read NEUT or GENIE output (same format)    
        // Generate the primary vertex
        RooTrackerNEUT.GeneratePrimaryVertex(anEvent);   
    }  
    
    else if(fGeneratorType=="ParticleGun"){
        ...
        */
        ...
        if (fTypeMomentum=="WidePion"){
            G4cout << "[ExN02PrimaryGeneratorAction] Momentum before update = " << aMomAmp << " MeV/c" << G4endl;
            aMomAmp = 50+(G4UniformRand()*(450.) );
            fParticleGun->SetParticleMomentum(aMomAmp*CLHEP::MeV);
            G4cout << "[ExN02PrimaryGeneratorAction] Momentum after update = " << aMomAmp << " MeV/c" << G4endl;
