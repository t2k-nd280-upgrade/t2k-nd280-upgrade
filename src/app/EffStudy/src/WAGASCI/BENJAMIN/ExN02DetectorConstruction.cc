...
G4VPhysicalVolume* ExN02DetectorConstruction::Construct()
{
  ...
  */
  ...
  //ND280WAGASCIActiveConstructor * WAGASCI = new ND280WAGASCIActiveConstructor(cNameSolidWorld+"/WAGASCI",this);

  //G4ThreeVector ModulePosition = GetTargetPos2();
  G4ThreeVector ModuleSize (GetTargetFullWidth2(),GetTargetFullHeight2(),GetTargetFullLength2());
  //First, create the target. It will be the mother volume
  ...
  physiTarget2 = new G4PVPlacement(0,GetTargetPos2(),logicTarget2,cNamePhysiTarget2,logicBasket,false,0);

  //ND280WAGASCIActiveConstructor * WAGASCI = new ND280WAGASCIActiveConstructor();
  //WAGASCI->ND280WAGASCIActiveConstructor::Construct(logicTarget2,GetTargetPos2(),ModuleSize,cNamePhysiTarget2);

  ND280WaffleActiveConstructor * Waffle = new ND280WaffleActiveConstructor();
  Waffle->ND280WaffleActiveConstructor::Construct(logicTarget2,GetTargetPos2(),ModuleSize,cNamePhysiTarget2);

  //G4VPhysicalVolume * WAGASCIVolume = WAGASCI->ND280WAGASCIActiveConstructor::Construct(logicWorld,logicTarget2,ModulePosition,ModuleSize);

  //new G4PVPlacement(&(ND280WAGASCI::Offsets::WAGASCIRotation),ND280WAGASCI::Offsets::WAGASCIPosition,TargetLV,"TargetWater",WorldLV,false,0,false);
  //G4VPhysicalVolume * WAGASCIVolume = 

  /*
  //Second, create the Grid.
  
  //G4LogicalVolume * WAGASCIVolume = WAGASCI->ND280WAGASCIActiveConstructor::Construct(ModulePosition,ModuleSize);
  //new G4PVPlacement(0,GetTargetPos2(),WAGASCIVolume,"TargetWater",logicWorld,false,0,false);
    ...
    */
    G4cout << "Target 2: " << G4endl
	   ...
	   << GetTargetFullLength2()/mm << " (length) mm^3" << G4endl;
    G4cout << " - position: ( " 
	   << GetTargetPos2().x()/mm << ", "
	   << GetTargetPos2().y()/mm << ", "
...
#endif
#ifdef DEBUG
  G4cout << "Set the sensitive detectors" << G4endl;
#endif
...
void ExN02DetectorConstruction::DefineMaterials() {
  ...
  WAGASHIScint->AddElement(elC, 8);
  WAGASHIScint->AddElement(elH, 8);
  ...
  WAGASHIScint_Empty->AddElement(elC, 8);
  WAGASHIScint_Empty->AddElement(elH, 8);
