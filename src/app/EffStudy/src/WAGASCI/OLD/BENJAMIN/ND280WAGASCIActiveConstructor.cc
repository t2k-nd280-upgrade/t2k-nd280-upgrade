...
void ND280WAGASCIActiveConstructor::Construct(G4LogicalVolume * TargetLV, G4ThreeVector ModulePosition, G4ThreeVector ModuleSize, const G4String RepositoryName){
  ...
  // No submodule is assumed. The plans are created one after another until it is not 
  // possible anymore due to target space limitation.
  // As for within one layer, we have added a gap between the layers and also, an offset 
  // between the target and the first (and therefore, last) scintillator layer.
  ...
  //const double LayerGap = 1000.0 * mm;
  ...
  // 
  // We have added a possibility of offset of the first (and therefore, last) 
  // scintillator in the layer. Note that there is also Gap between the scintillators.
  //
  ...
  G4cout << "Vertical plan layer (=vertical scintillator):" << G4endl 
	 << "TargetHeight=" << TargetHeight 
	 << ", Layer Height = " << LayerPlanHeight 
	 << ", Initial layer offset = "<< InitialLayerHeightPlanFirstScintiOffset 
	 <<", final layer offset = " << LayerHeightPlanFirstScintiOffset << G4endl; 
  G4cout << "Test of the plan vertical scintillator length = " << ScintiVerticalPlanLength 
	 << " and if math are correct, should be equal to " << LayerPlanHeight + 2*(LayerHeightPlanFirstScintiOffset-InitialLayerHeightPlanFirstScintiOffset) << G4endl;
  G4cout << "Horizontal plan layer (=horizontal scintillator):" << G4endl 
	 << "TargetWidth=" << TargetWidth 
	 << ", Layer Width = " << LayerPlanWidth 
	 << ", Initial layer offset = "<< InitialLayerWidthPlanFirstScintiOffset 
	 <<", final layer offset = " << LayerWidthPlanFirstScintiOffset << G4endl; 
  G4cout << "Test of the plan horizontal scintillator length = " << ScintiHorizontalPlanLength 
	 << " and if math are correct, should be equal to " << LayerPlanWidth + 2*(LayerWidthPlanFirstScintiOffset-InitialLayerWidthPlanFirstScintiOffset) << G4endl;

  G4cout << "Vertical grid layer (=vertical scintillator):" << G4endl 
	 << "TargetHeight=" << TargetHeight 
	 << ", Layer Height = " << LayerGridHeight 
	 << ", Initial layer offset = " << InitialLayerHeightGridFirstScintiOffset 
	 <<", final layer offset = " << LayerHeightGridFirstScintiOffset << G4endl; 
  G4cout << "Test of the grid vertical scintillator length = " << ScintiVerticalGridLength 
	 << " and if math are correct, should be equal to " << LayerGridHeight + 2*(LayerHeightGridFirstScintiOffset-InitialLayerHeightGridFirstScintiOffset) << G4endl;
  G4cout << "Horizontal grid layer (=horizontal scintillator):" << G4endl 
	 << "TargetWidth=" << TargetWidth 
	 << ", Layer Width = " << LayerGridWidth 
	 << ", Initial layer offset = "<< InitialLayerWidthGridFirstScintiOffset 
	 <<", final layer offset = " << LayerWidthGridFirstScintiOffset << G4endl; 
  G4cout << "Test of the grid horizontal scintillator length = " << ScintiHorizontalGridLength 
	 << " and if math are correct, should be equal to " << LayerGridWidth + 2*(LayerWidthGridFirstScintiOffset-InitialLayerWidthGridFirstScintiOffset) << G4endl;
#endif
  ...
  const double SlitSafetyMarginWidth = 1 * mm;
  const double SlitSafetyMarginHeight = 1 * mm;
  const double SlitSafetyMarginThickness = 1 * mm;
...
*/
  ...
  G4Box * ScintiHorizontalPlanBox 
    = new G4Box("ScintiHorizontalPlanBox",
		ScintiHorizontalPlanLength/2.,
		ScintiPlanWidth/2.,
		ScintiPlanThickness/2.);
  
  //Second, create the LV of a template scintillator: 
  G4LogicalVolume * ScintiHorizontalPlanLV 
    = new G4LogicalVolume(ScintiHorizontalPlanBox,
			  Scintillator,
			  //FindMaterial("WAGASHIScintillator"),
			  "ScintiPlanLV");

  //2. Layer plan vertical
  //First, create the volume of a template scintillator:
  G4Box * ScintiVerticalPlanBox 
    = new G4Box("ScintiVerticalPlanBox",
		ScintiPlanWidth/2.,
		ScintiVerticalPlanLength/2.,
		ScintiPlanThickness/2.);
  
  //Second, create the LV of a template scintillator: 
  G4LogicalVolume * ScintiVerticalPlanLV 
    = new G4LogicalVolume(ScintiVerticalPlanBox,
			  Scintillator,
			  //FindMaterial("WAGASHIScintillator"),
			  "ScintiPlanLV");

  // Currently, the problem is that I do not have created a layer LV that allow me to place a layer anywhere I wish. 
  // So we will work with scintillator planes instead of layers. So we should create also the LV of other scintillator types.
  ...
  G4Box * ScintiHorizontalGridExternalBox 
    = new G4Box("ScintiHorizontalGridExternalBox",
		ScintiHorizontalGridLength/2.,
		ScintiGridThickness/2.,
		ScintiGridWidth/2.);
  
  G4VSolid * ScintiHorizontalGridBox 
    = (G4VSolid*) ScintiHorizontalGridExternalBox ;
  
  G4VSolid* SlitHorizontalGrid 
    = new G4Box("SlitHorizontalGrid",
		SlitWidth,
		SlitThickness,
		SlitHeight);
...
#ifdef DEBUG
  G4cout << "Horizontal GRID scintillator, dimensions = (" << ScintiHorizontalGridExternalBox->GetXHalfLength()*2 
	 << ", " << ScintiHorizontalGridExternalBox->GetYHalfLength()*2 
	 << ", " << ScintiHorizontalGridExternalBox->GetZHalfLength()*2 << ") " 
	 << G4endl;
  G4cout << "Number of slits = " << NumberOfSlitsHorizontal 
  ...
  double PositionAlongBar = 
    LayerWidthGridFirstScintiOffset - 
    InitialLayerWidthGridFirstScintiOffset + 
    ScintiGridThickness/2 - 
    ScintiHorizontalGridExternalBox->GetXHalfLength();
  ...
  G4LogicalVolume * ScintiHorizontalGridLV 
    = new G4LogicalVolume(ScintiHorizontalGridBox,
			  Scintillator,
			  //FindMaterial("WAGASHIScintillator"),
			  "ScintiGridLV");
  ...
  G4Box * ScintiVerticalGridExternalBox 
    = new G4Box("ScintiVerticalGridExternalBox",
		ScintiGridThickness/2.,
		ScintiVerticalGridLength/2.,
		ScintiGridWidth/2.);
  
  G4VSolid * ScintiVerticalGridBox = (G4VSolid*) ScintiVerticalGridExternalBox;
  
  G4VSolid* SlitVerticalGrid 
    = new G4Box("SlitVerticalGrid",
		SlitThickness,
		SlitWidth,
		SlitHeight);
  ...
  //Iterate the slit removal in fact. Take the box, remove one slit, update the box with the box having one slit removed, remove another slit etc...
#ifdef DEBUG
  G4cout << "Vertical GRID scintillator, dimensions = (" << ScintiVerticalGridExternalBox->GetXHalfLength()*2 << ", " 
	 << ScintiVerticalGridExternalBox->GetYHalfLength()*2 << ", " 
	 << ScintiVerticalGridExternalBox->GetZHalfLength()*2 << ") " 
	 << G4endl;
  G4cout << "Number of slits = " << NumberOfSlitsVertical 
	 << ", Dimensions of the slits = ("<< SlitThickness << ", " 
	 << SlitWidth << ", " 
	 << SlitHeight << ") " 
	 << G4endl;
#endif
  ...
  PositionAlongBar = 
    LayerHeightGridFirstScintiOffset - 
    InitialLayerHeightGridFirstScintiOffset + 
    ScintiGridThickness/2 - 
    ScintiVerticalGridExternalBox->GetYHalfLength();
  ...
  for(int i=0;i<NumberOfSlitsVertical;i++){
   ...
   G4LogicalVolume * ScintiVerticalGridLV 
     = new G4LogicalVolume(ScintiVerticalGridBox,
			   Scintillator,
			   //FindMaterial("WAGASHIScintillator"),
			   "ScintiGridLV");
 ...
 for(int m=0;m<NumberOfModules;m++){//Loop over the number of modules (water/air etc...)
   ...
   while(1){
	 ...
	 new G4PVPlacement(0, 
			   ...
			   ScintiVerticalPlanLV,
			   RepositoryName+Form("LayerPlanVerticalScinti[%d][%d][%d]",m,NumberOfLayerPlanVertical,i),
			   TargetLV,
			   false,
			   0,
			   false);
...
#ifdef DEBUG
	  G4cout << "Scintillator position = (" 
		 << ScintillatorPosition[0] << ", " 
		 << ScintillatorPosition[1] << ", " 
		 << ScintillatorPosition[2] << ") " 
		 << G4endl;
#endif
...
#ifdef DEBUG
	G4cout << "Layer #" << NumberOfLayers
	       << " is of type grid, number of scintillators vertical grid= " << LayerGridNumberOfScintiVertical
	       << ", horizontal grid= " << LayerGridNumberOfScintiHorizontal
	       << ", Layer width = " << LayerGridWidth
	       << ", Layer height = " << LayerGridHeight
	       << G4endl;
#endif
	...
	for(int i=0; i<LayerGridNumberOfScintiVertical;i++){//Loop over scintillators
	  ...
	  new G4PVPlacement(0,
			    ScintillatorPosition,
			    ScintiVerticalGridLV,
			    RepositoryName+Form("LayerGridVerticalScinti[%d][%d][%d]",m,NumberOfLayerGrid,i),
			    TargetLV,
			    false,
			    0,
			    false);	 
...
#ifdef DEBUG
	  G4cout << "Vertical grid, Scintillator position = (" 
		 << ScintillatorPosition[0] << ", " 
		 << ScintillatorPosition[1] << ", " 
		 << ScintillatorPosition[2] << ") " 
		 << G4endl;
	...
	for(int i=0; i<LayerGridNumberOfScintiHorizontal;i++){//Loop over scintillators
	  ...
	  new G4PVPlacement(0, 
			    ScintillatorPosition, 
			    ScintiHorizontalGridLV,
			    RepositoryName+Form("LayerGridHorizontalScinti[%d][%d][%d]",m,NumberOfLayerGrid,i),
			    TargetLV,
			    false,
			    0,
			    false);
...
#ifdef DEBUG
	  G4cout << "Horizontal grid, Scintillator position = (" 
		 << ScintillatorPosition[0] << ", " 
		 << ScintillatorPosition[1] << ", " 
		 << ScintillatorPosition[2] << ") " 
		 << G4endl;
...
#ifdef DEBUG
	G4cout << "Layer #" << NumberOfLayers
	       << " is of type grid, number of scintillators = " << LayerPlanNumberOfScintiHorizontal
	       << G4endl;
#endif
	...
	for(int i=0; i<LayerPlanNumberOfScintiHorizontal;i++){//Loop over scintillators
	  ...
	  new G4PVPlacement(0, 
			    ScintillatorPosition,
			    ScintiHorizontalPlanLV,
			    RepositoryName+Form("LayerPlanHorizontalScinti[%d][%d][%d]",m,NumberOfLayerPlanHorizontal,i),
			    TargetLV,
			    false,
			    0,
			    false);       
