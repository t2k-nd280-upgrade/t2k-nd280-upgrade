#ifndef ND280_WAGASCI_CONSTRUCTOR_HH_SEEN
#define ND280_WAGASCI_CONSTRUCTOR_HH_SEEN
// $Id: ND280WAGASCIConstructor.hh,v 1.9 2017/28/3 dsgalabe $
//
...
class ND280WAGASCIConstructor : public ND280Constructor {
public:
  ND280WAGASCIConstructor(G4String n, ExN02DetectorConstruction* c)
    : ND280Constructor(n,c) {Init();};
  ND280WAGASCIConstructor(G4String n, ND280Constructor* p)
    : ND280Constructor(n,p) {Init();};
  virtual ~ND280WAGASCIConstructor();
  ...
  /// Set the width of the WAGASCI detector bounding box "along" the X axis
  void SetWidth(double w) {fWidth = w;}
  /// Get the width of the WAGASCI detector
  double GetWidth(void) {return fWidth;}

  /// Set the height of the WAGASCI detector bounding box "along" the Y axis  
  void SetHeight(double w) {fHeight = w;}
  /// Get the height of the WAGASCI detector
  double GetHeight(void) {return fHeight;}
  
  /// Set the length of the WAGASCI detector bounding box "along" the beam axis (the
  /// Z axis).  
  void SetLength(double w) {fLength = w;}
  /// Get the length of the WAGASCI detector
  ...
  /// Set the X position of the WAGASCI detector     
  void SetPosX(double pos) {fPosX = pos;}
  /// Get the X position of the WAGASCI detector  
  double GetPosX(void) {return fPosX;}
  
  /// Set the Y position of the WAGASCI detector       
  void SetPosY(double pos) {fPosX = pos;}
  /// Get the Y position of the WAGASCI detector   
  double GetPosY(void) {return fPosY;}
  
  /// Set the Z position of the WAGASCI detector 
  void SetPosZ(double pos) {fPosZ = pos;}
  // Get the Z position of the WAGASCI detector 
