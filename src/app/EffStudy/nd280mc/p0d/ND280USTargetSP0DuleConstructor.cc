...
#include "p0d/ND280USTargetSP0DuleConstructor.hh"
...
class ND280USTargetMessenger : public ND280ConstructorMessenger {

private:

    ND280USTargetSP0DuleConstructor* fConstructor;
...
public:
    ND280USTargetMessenger(ND280USTargetSP0DuleConstructor* c)
        : ND280ConstructorMessenger(c,"Control the USTarget geometry."),
          fConstructor(c) {
    ...
    virtual ~ND280USTargetMessenger() {
...
ND280USTargetSP0DuleConstructor::~ND280USTargetSP0DuleConstructor() {}

void ND280USTargetSP0DuleConstructor::Init(void) {
    ...
    // Set the ladder offsets.  These are the offsets from the top of the MPPC
    // shrouds.
    ...
    SetMessenger(new ND280USTargetMessenger(this));
}

double ND280USTargetSP0DuleConstructor::GetLength() {
    ...
    length += 13*(radiator.GetLength()+GetWastedSpace());

    target.NoBackCover();
    length += 12*target.GetLength();

    target.MakeBackCover();
    length += target.GetLength();

    return length;
}

double ND280USTargetSP0DuleConstructor::GetWidth() {
    ND280P0DuleConstructor& p0dule 
        =  Get<ND280P0DuleConstructor>("P0Dule");
...
double ND280USTargetSP0DuleConstructor::GetHeight() {
    ND280P0DuleConstructor& p0dule 
        =  Get<ND280P0DuleConstructor>("P0Dule");
...
void ND280USTargetSP0DuleConstructor::SetP0DuleTrans(int mod, 
                                                  const G4ThreeVector& trans) {
  if (mod < 0){
    //G4Exception("P0Dule number out of range");
    G4Exception("ND280USTargetSP0DuleConstructor::SetP0DuleTrans",
...
G4ThreeVector ND280USTargetSP0DuleConstructor::GetP0DuleTrans(int mod) const {
  if (mod < 0){
    //G4Exception("P0Dule number out of range");
    G4Exception("ND280USTargetSP0DuleConstructor::GetP0DuleTrans",
		"MyCode0002",FatalException,"P0Dule number out of range");
  }
  if (mod > 13){
    //G4Exception("P0Dule number out of range");
...
G4LogicalVolume *ND280USTargetSP0DuleConstructor::GetPiece(void) {
    ND280P0DuleConstructor& p0dule 
        =  Get<ND280P0DuleConstructor>("P0Dule");
    ND280P0DTargetRadiatorConstructor& radiator
        = Get<ND280P0DTargetRadiatorConstructor>("Radiator");
    ...
    for (int i=0; i<13; ++i) {
        if (i==12) target.MakeBackCover();
        else target.NoBackCover();
    ...
    double upstreamLadder = 9*CLHEP::cm;
    double downstreamLadder = 7*CLHEP::cm;
    new G4PVPlacement(0,                    // no rotation
                      G4ThreeVector(0,
                                    - GetHeight()/2
                                    + p0dule.GetHeight()
    ...
    target.MakeBackCover();
    double withCover = target.GetLength();
    target.NoBackCover();
    double withoutCover = target.GetLength();
    // ND280Log("   US Target Water Thickness:    " << withoutCover/mm << " mm");
    // ND280Log("   US Target CoverThickness;     " 
    //          << (withCover - withoutCover)/mm << " mm");
    G4cout << "   US Target Water Thickness:    " << withoutCover/CLHEP::mm << " mm" << G4endl;
    G4cout << "   US Target CoverThickness;     " 
	   << (withCover - withoutCover)/CLHEP::mm << " mm" << G4endl;
