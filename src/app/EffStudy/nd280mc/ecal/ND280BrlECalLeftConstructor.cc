#include <ND280BrlECalLeftConstructor.hh>
...
void ND280BrlECalLeftConstructor::Init() {
  SetLengthX(4140*CLHEP::mm);
  SetWidthY(2503*CLHEP::mm);
  SetHeightZ(492*CLHEP::mm);
    AddConstructor(new ND280BrlECalActiveConstructor("Active",this));
    Get<ND280BrlECalActiveConstructor>("Active").SetIsSide(true);
}

G4LogicalVolume* ND280BrlECalLeftConstructor::GetPiece() {
    ...
    G4LogicalVolume* base_logical 
        = components.CarbonPanel(GetLengthX(), base_y, base_z,
                                 ...
                                 ND280ECal::Brl::CarbonFrameYSide);
    ...
    double lid_z = ND280ECal::Brl::LidThicknessSide;
    double lid_pos_y = plate_y / 2.0;
    double lid_pos_z = (GetHeightZ() - lid_z) / 2.0;

    G4LogicalVolume* lid_logical 
        = components.CarbonPanel(GetLengthX(), lid_y, base_z,
                                 ND280ECal::Brl::CarbonFrameX,
                                 ND280ECal::Brl::CarbonFrameYSide);

    new G4PVPlacement(0,  // rotation
                      G4ThreeVector(0.0, lid_pos_y, lid_pos_z),  // position
    ...
    // Primary Bulkheads
    const double bulk_z = GetHeightZ() - ND280ECal::Brl::LidThicknessSide
        - ND280ECal::Brl::BaseThicknessSide;
    ...
    double bulk_para_pos_y = ((GetWidthY() - bulk_thick) / 2.0) 
        - ND280ECal::Brl::BulkheadPrimaryInsetSide;

    double bulk_perp_y = GetWidthY() - ND280ECal::BulkheadPlateThickness 
        - bulk_thick - ND280ECal::Brl::BulkheadPrimaryInsetSide;
    ...
    double cp_height = GetHeightZ() - ND280ECal::Brl::LidThicknessSide 
        - ND280ECal::Brl::BaseThicknessSide;
    ...
    double cppara_pos_y = ((GetWidthY() - ND280ECal::CoolingPlateBaseWidth) / 2.0)
        - ND280ECal::CoolingPlateBrlSideInset;
    ...
    new G4PVPlacement(cppara_rotation,  // rotation
                      G4ThreeVector(cppara_pos_x, cppara_pos_y, cp_pos_z),  // position
