...
private:
    ...
    //Extra information: (not considered in pre_processing)

    Int_t         fMulti;                           //Number of times the hit is used to reconstruct 3D voxels
    std::vector<ND280SFGDVoxel*>  fVoxels;          //List of pointers to the voxels intersected by the fiber where the hit has been measured.
...
public:
    ...
    void SetVoxels          (std::vector<ND280SFGDVoxel*> p_vxls) { fVoxels         = p_vxls;           }
    void AddVoxel           (ND280SFGDVoxel* p_vxl)               { fVoxels.push_back(p_vxl);           }
    void RmVoxel            (Int_t p_vxl_index)         { fVoxels.erase(fVoxels.begin() + p_vxl_index); }   
