...
void IngridPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  // Define neutrino interaction vertex
	...
	prob=ingrid_region/scibar_region;
      ...
      if(prob<(1-G4UniformRand()))continue;

      
      // unit of pos is [cm];
      if( vertex_flag==0 ) {
        flayer = (int)(2*(1-G4UniformRand()));     // 0 < 2*rand < 2
	pos[2] = width_ingrid*(1-G4UniformRand());
        pos[2] = pos[2] + Pscinti_start + flayer*distance_pln;    
      }
      else if( vertex_flag==1 ) {
        flayer = (int)(34*(1-G4UniformRand()));     // 0 < 34*rand < 34
        pos[2] = width_scibar*(1-G4UniformRand());
        pos[2] = pos[2] + Pscinti_start + distance_pln + diff + distance_first + flayer*distance_pln;
      }
      else if( vertex_flag==2){
	scitype= (int)((1-G4UniformRand())/scibar_width*(ingrid_width+scibar_width));
	flayer = (int)(17*(1-G4UniformRand()));     // 0 < 17*rand < 17
	if(scitype==0){
	  pos[2] = width_scibar*(1-G4UniformRand());
