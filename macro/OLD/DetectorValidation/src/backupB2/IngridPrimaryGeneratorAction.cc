...
void IngridPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  // Define neutrino interaction vertex
    ...
    // jnubeam ndid = 7 (INGRID-upstream surface for BG study)
      ...
      G4double lx       = pos[0] - HallX; // vertex X from hall center
      G4double z_offset = 1.0*sqrt( HallRadiusMax * HallRadiusMax - lx * lx ); // vertex X from hall center
      G4double radius_tmp = HallRadiusMin + (HallRadiusMax-HallRadiusMin)/HallRadiusMin * G4UniformRand();
      //pos[2]         = (470.* G4UniformRand() - z_offset+HallZ);
      pos[2] = 1.0*sqrt(radius_tmp*radius_tmp - lx*lx) - HallZ;

      cout << "pos[0]    :" << pos[0]   <<endl 
           << "lx        :" << lx       <<endl 
           << "z_offset  :" << z_offset <<endl 
           << "pos[2]    :" << pos[2]   <<endl ;
