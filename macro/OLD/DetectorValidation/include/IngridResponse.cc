...
const G4double scilen_B2 = 250.;//////////////B2
const G4double scishort_B2 = 150.;////////////B2
...
void IngridResponse::ApplyLightCollection(G4double* edep, G4int mod, G4int view, G4ThreeVector pos, G4int pln){ //added for B2 2014/1/27
...
/////////B2////////////////////
if(mod==1 || mod==3 ){
  //if( view==topview ) x = fabs(scilen_B2/2. + pos[0]/cm);
  //else if( view==sideview ) x = fabs(scilen_B2/2. + pos[1]/cm);
  x=0;//2014/6/11
//std::cout << pos[0]/cm << " " << pos[1]/cm  << "\n"; 
}

if(mod==2){
  //if( view==topview ) x = fabs(scilen_B2/2. + pos[2]/cm);
  //else if( view==sideview ) x = fabs(scilen_B2/2. + pos[1]/cm);
  x=0;//2014/6/11
}
...
if(mod==5){
  /*
  if(pln>3 && pln<51){
    if( view==topview ) x = fabs(scishort_B2/2. + pos[0]/cm);
    else if( view==sideview ) x = fabs(scishort_B2/2. + pos[1]/cm);
  }
  else{
    x=2.5;
  }
...
///////////////////////////////
  ...
  if(mod!=5 || (pln<4 || pln>50)){
    i=x/5;
    x=fabs(x-i*5-2.5);
    x=0;//2014/6/11
    //*edep *= exp(-1.*x/sciattleng);
    //*edep *= exp(-1.*x/sciattleng)*SciBarFactor;//for B2 2014/1/28 //2014/6/11
    *edep *= exp(-1.*x/sciattleng);//2014/7/4
  }
  else{
    i=x/2.5;
    x=fabs(x-i*2.5-1.25);
    x=0;//2014/6/11
    //*edep *= exp(-1.*x/sciattleng);               //for B2 2014/1/28
    //*edep *= exp(-1.*x/sciattleng)*SciBarFactor;//2014/6/11
    *edep *= exp(-1.*x/sciattleng);//2014/7/4
  }
...
void IngridResponse::ApplyFiberResponse(G4double* edep,G4int mod, G4double* time, G4int view, G4ThreeVector pos, G4int pln)//added for B2 2014/1/27
{
G4double x = 0.;
...
/////////B2////////////////////
if(mod==1 || mod==3){
  //if( view==topview ) x = fabs(scilen_B2/2. - pos[1]/cm);
  //else if( view==sideview ) x = fabs(scilen_B2/2. + pos[0]/cm);
  x=0;//2014/6/11
}
if(mod==2){
  //if( view==topview ) x = fabs(scilen_B2/2. - pos[1]/cm);
  if( view==topview ) x = 0;
  else if( view==sideview ) x = fabs(150 + pos[2]/cm); 
  //x=0;//2014/6/11
}
if(mod==4){
  //if( view==topview ) x = fabs(scilen_B2/2. + pos[0]/cm);
  //else if( view==sideview ) x = fabs(scilen_B2/2. + pos[1]/cm); 
  //x=0;
  if( view==topview && pln>=40 && pln<60){ x = fabs(112.5 + pos[2]/cm); }
  else if( view==sideview && pln>=20 && pln<40 ){ x = fabs(105 + pos[2]/cm); }
  else x=0;
}
if(mod==5){
  //if(pln<24){
/*
  if(pln>3 && pln<51){
    if( view==topview ) x = fabs(scishort_B2/2. - pos[1]/cm);
    else if( view==sideview ) x = fabs(scishort_B2/2. + pos[0]/cm);
  }
  else{
    x=0;
  }
...
///////////////////////////////
  ...
  *edep *= exp(-1.*0./attleng);
  //if(mod==2 && view==0)G4cout << x << " " << *time ;
  // delay in fiber
  *time += TransTimeInFiber*x;
  *time = gRandom->Gaus(*time,1.0);

  //if(mod==2 && view==0)G4cout << " " << *time << "\n" ;
return;
