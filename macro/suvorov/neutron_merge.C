...
void neutron_merge() {
	TFile* file = new TFile("/t2k/users/suvorov/AnalysisResults/ndUP/SuperFGD/SuperFGD-neutron_v7-UseXY-UseXZ-UseYZ-Separate10_na_1000000.root", "READ");
  ...
  for (Int_t distID = 0; distID < Ndist; ++distID) {
    ...
    for (Int_t i = 0; i < 7; ++i) {
      energy_resol[i] = new TH2F(Form("resol_mac_%i", i), "energy resolution", 250, 0., 500, 300, 0., 600);
    ...
    for (Int_t entryID = 0; entryID < NEntries; ++entryID) {
      ...
      time = gen->Gaus(first_hit_time, 0.9);
      beta = dist_cubes / (time * 30.);
      ekin2 = sqrt(939.565379*939.565379 / (1 - beta*beta)) - 939.565379;
      energy_resol[1]->Fill(ekin, ekin2);
      energy_resol_rebin[1]->Fill(ekin, ekin2);
  
      time = gen->Gaus(first_hit_time, 0.9/sqrt(2));
      beta = dist_cubes / (time * 30.);
      ekin2 = sqrt(939.565379*939.565379 / (1 - beta*beta)) - 939.565379;
      energy_resol[2]->Fill(ekin, ekin2);
      energy_resol_rebin[2]->Fill(ekin, ekin2);
  
      time = gen->Gaus(first_hit_time, 0.9/sqrt(3));
      beta = dist_cubes / (time * 30.);
      ekin2 = sqrt(939.565379*939.565379 / (1 - beta*beta)) - 939.565379;
      energy_resol[3]->Fill(ekin, ekin2);
      energy_resol_rebin[3]->Fill(ekin, ekin2);
  
      time = gen->Gaus(first_hit_time, 0.6);
      beta = dist_cubes / (time * 30.);
      ekin2 = sqrt(939.565379*939.565379 / (1 - beta*beta)) - 939.565379;
      energy_resol[4]->Fill(ekin, ekin2);
      energy_resol_rebin[4]->Fill(ekin, ekin2);
  
      time = gen->Gaus(first_hit_time, 0.6/sqrt(2));
      beta = dist_cubes / (time * 30.);
      ekin2 = sqrt(939.565379*939.565379 / (1 - beta*beta)) - 939.565379;
      energy_resol[5]->Fill(ekin, ekin2);
      energy_resol_rebin[5]->Fill(ekin, ekin2);
  
      time = gen->Gaus(first_hit_time, 0.6/sqrt(3));
      beta = dist_cubes / (time * 30.);
      ekin2 = sqrt(939.565379*939.565379 / (1 - beta*beta)) - 939.565379;
      energy_resol[6]->Fill(ekin, ekin2);
      energy_resol_rebin[6]->Fill(ekin, ekin2);
    }
    ...
    TDirectory* dir = file_out->mkdir(Form("CutValue_%f", distance_cut[distID]));
    dir->cd();
    ...
    for (Int_t resID = 0; resID < 7; ++resID ) {
      ...
      for (Int_t i = 2; i <= Nbins; ++i) {
        ...
        std::cout << graph[resID]->GetN() << "\t" << x_center << "\t" << mean << "\t" << sigmaL << "\t" << sigmaR << "\t" <<  sigmaR / mean *100. << std::endl;
      }
      ...
      graph[resID]->Draw("ap");
      gPad->Update();
      graph[resID]->Write(Form("graph%i", resID));
    } // loop over projections to fit
