...
//    G4cout << "flavor " << neutrino_flavor_tmp << "\n";
  ...
  // Define neutrino interaction vertex
    ...
    //cout<<"fdid="<<fdid<<endl;
    // X-Y vertex
    pos[0] = (neut->Vector).Neutrino.x*2.3/2;//HK: then add weighting factor in the end
    pos[1] = (neut->Vector).Neutrino.y*2.3/2;//HK: same
...
//B2//////////////////////////////////
    ...
    if( (fdid==7||fdid==10) && module_mode==5) {//change 2to5
	//pos[2] = -75.0 + (G4UniformRand())*150.0;
	pos[2] = -100.0 + (G4UniformRand())*50.0;//2014/6/11//HK
    }
...
//for B2
    if((fdid==7||fdid==10) && module_mode == 5 ) {//change 2to5////////////////////
	//if(fabs(pos[0])<=75 && fabs(pos[1]) <=75){
	if(fabs(pos[0])<=115. && fabs(pos[1]) <=115.){//2014/6/11 //HK
	ID = 19;
	goto NEXTSTEP;
	}
    }
