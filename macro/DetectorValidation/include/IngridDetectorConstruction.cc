...
//////////////////////////////////////////////////////////////
 ...
 ////////////////////////////////////////////////////////// 
  ...
  for(int i=0;i<latice_sciNumX;i++){
  ...
  //G4Box* Water_box = new G4Box("Water_box",230*cm,230*cm,50*cm);//2013/6/11
  G4LogicalVolume *WaterLV = new G4LogicalVolume(Water_box,Water,"WaterLV");
  G4VisAttributes* WaterVisAtt = new G4VisAttributes(G4Colour(0.,1.,1.));
  ...
  for(int k=start_mod;k<stop_mod;k++){
    ...
    //WAGASCI
      ...
      */
      ...
      G4double Gap2; 
      Gap2=plnSpace*mm;
      //std::cout<<HbarLenX+subHBarX<<std::endl;
      std::cout<<"************************** Number of scinti n each 2D plane x="<<plan2D_sciNumX<<", lattice="<<plan2D_sciNumX<<std::endl;
      std::cout<<"************************** Number of scinti n each 2D plane y="<<plan2D_sciNumY<<", lattice="<<plan2D_sciNumY<<std::endl;
      for(int i=0;i<latice_plnNum+1;i++){//2014/6/11
        G4double z = -(waterLenZ/2.)*mm + (HbarLenY/2.)*mm + Gap2*i + firstdistance_mod5z*mm;//2014/6/11
        for(int j=0;j<plan2D_sciNumX;j++){//2014/6/11 
	  char name[30]; 
          G4double x_scinti =  2*j*scibar2_y +offset_mod5_v + scibar2_y;//2014/7/4 
          //G4double x_scinti =  2*j*scibar2_y +offset_mod5_v;//HK
	  ...
	  char name[30]; 
          ...
          //G4double y_scinti =  2*j*scibar2_y +offset_mod5_h;//HK
      ...
      std::cout<<"************************** Number of scinti n each grid x="<<latice_sciNumX<<", lattice="<<latice_sciNumX<<std::endl;
      std::cout<<"************************** Number of scinti n each grid y="<<latice_sciNumY<<", lattice="<<latice_sciNumY<<std::endl;

      for(int pln=0;pln<latice_plnNum;pln++){
 	        //positioning 2D lattice 
 	 	for(int j=0;j<latice_sciNumX;j++){
  		...
  		for(int j=0;j<latice_sciNumY;j++){
...
void IngridDetectorConstruction::DefineStructures()
{
  ...
  //Scibar type for proton module added by kikawa
  scibar2_y=1.25*cm;        // 2.5cm wide HK
  ...
  //B2
  offset_mod5_v = -115.0*cm;//HK -50-> -100
  offset_mod5_h = -115.0*cm;//HK -50-> -100
