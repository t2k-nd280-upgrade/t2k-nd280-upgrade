...
G4LogicalVolume* ND280P0DECalLeftConstructor::GetPiece() {
    ...
    const double active_pos_y = -(GetWidthY()/2.0) + (active.GetWidthY()/2.0) 
        + plate_y;
    const double active_pos_z = base_pos_z 
        + (base_z/2.0) + (active.GetHeightZ()/2.0);
    ...
    const double bulk_down_z = fModuleHeightZ 
        - ND280ECal::P0D::LidThickness - ND280ECal::P0D::BaseThickness;
    ...
    G4LogicalVolume* bulk_down = components.Bulkhead(bulk_down_x, 
                                                     bulk_down_y,
                                                     bulk_down_z);
    ...
    const double bulk_para_z = fModuleHeightZ 
        - ND280ECal::P0D::LidThickness - ND280ECal::P0D::BaseThickness;
